...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateMachineType() {
    if (["micro", "small", "medium"].includes(type.value)) {
      vcpu.disabled = true;
      vcpu.value = "";
      full.value = `${family.value}-${type.value}`;
    } else {
      vcpu.disabled = false;
      if (family.value && type.value && vcpu.value) {
        full.value = `${family.value}-${type.value}-${vcpu.value}`;
      } else {
        full.value = "";
      }
    }
  ...
  family.addEventListener("change", updateMachineType);
